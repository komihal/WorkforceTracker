...
// Дедупликация событий onLocation (предотвращение дублирования событий)
let lastLocationEventHash = '';
let lastLocationEventTime = 0;
const LOCATION_EVENT_DEDUP_MS = 1000; // 1 секунда окно дедупликации

// Throttle для мониторинговых отправок onLocation (во избежание дублей и спама)
...
export async function initLocation() {
  ...
  try {
    ...
    BGGeo.onHttp(async (r) => {
      // Мониторинг встроенного uploader'а Transistorsoft
      ...
      if (r.status === 200) {
        ...
        console.log('[BG][onHttp] Native uploader data sent successfully');
      } else {
